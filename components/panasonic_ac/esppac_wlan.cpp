...
void PanasonicACWLAN::control(const climate::ClimateCall &call) {
  ...
  if (call.has_custom_fan_mode()) {
    ...
    // ESPHome 2026.x: get_custom_fan_mode() returns StringRef -> use c_str() (or strcmp) for comparisons
  ...
  if (call.has_custom_preset()) {
    ...
    // ESPHome 2026.x: get_custom_preset() returns StringRef -> use c_str() (or strcmp) for comparisons
...
void PanasonicACWLAN::handle_init_packets() {
  if (this->state_ == ACState::Initializing) {
    if (millis() - this->init_time_ > INIT_TIMEOUT) {
      ...
      this->state_ = ACState::Handshake;
    }
  } else if (this->state_ == ACState::FirstPoll && millis() - this->last_packet_sent_ > FIRST_POLL_TIMEOUT) {
    ESP_LOGD(TAG, "Polling for the first time");
    send_command(CMD_POLL, sizeof(CMD_POLL));

    this->state_ = ACState::HandshakeEnding;
  } else if (this->state_ == ACState::HandshakeEnding && millis() - this->last_packet_sent_ > INIT_END_TIMEOUT) {
...
bool PanasonicACWLAN::verify_packet() {
  ...
  if (this->rx_buffer_[0] == 0x66) {
    ESP_LOGI(TAG, "Received sync packet, triggering initialization");
    this->init_time_ -= INIT_TIMEOUT;
    this->rx_buffer_.clear();
    return false;
  }

  if (this->rx_buffer_[0] != HEADER) {
    ESP_LOGW(TAG, "Dropping invalid packet (header)");
    this->rx_buffer_.clear();
    return false;
  }

  if (this->state_ == ACState::Ready && this->waiting_for_response_) {
    if (this->rx_buffer_[1] != this->transmit_packet_count_ - 1 && this->rx_buffer_[1] != 0xFE) {
      ESP_LOGW(TAG, "Correcting shifted tx counter");
      this->receive_packet_count_ = this->rx_buffer_[1];
    }
  } else if (this->state_ == ACState::Ready) {
...
void PanasonicACWLAN::handle_packet() {
  ...
  } else if (this->rx_buffer_[2] == 0x10 && this->rx_buffer_[3] == 0x89) {
  ...
  } else if (this->rx_buffer_[2] == 0x10 && this->rx_buffer_[3] == 0x0A) {
...
void PanasonicACWLAN::send_set_command() {
  ...
  packet[4] = 0x00;
  packet[5] = (4 * this->set_queue_index_) - 1 + 6;
  packet[6] = 0x01;
  packet[7] = 0x01;
...
void PanasonicACWLAN::send_packet(std::vector<uint8_t> packet, CommandType type) {
  ...
  if (type == CommandType::Response)
    ...
    packetCount = this->transmit_packet_count_ - 1;
